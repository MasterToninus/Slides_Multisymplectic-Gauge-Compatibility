...
\usepackage{pdfpages} %copy from other sources
...
\begin{frame}[fragile]{A \emph{pentagonal} diagram in symplectic geometry}
	...
	\begin{minipage}[t][8.5em][t]{\textwidth}
		...
		\begin{itemize}
			...
			\item<3->[-] A Lie algebroid is a "controlled" $\infty$-dimensional \alert<3>{Lie algebra} w/
			\begin{displaymath}
...
\begin{frame}[fragile]{A \emph{pentagonal} diagram in symplectic geometry}
	...
	~~ \alert{($\widetilde{\omega} = \omega - \dd B$)}
	%
	\begin{center}
	...
	\begin{minipage}[t][8em][t]{\textwidth}
		\begin{itemize}
			...
			\only<6-8>{
				...
				\item<7-> let be $f:\mathfrak{g} \to C^\infty(M)_\omega$ and $\tilde{f}:\mathfrak{g} \to C^\infty(M)_{\tilde{\omega}}$ two \alert<7>{comoment maps} s.t.
				\begin{displaymath}
...
\begin{frame}[fragile]{A \emph{pentagonal} diagram in symplectic geometry}\label{frame:pentagonMotivation}
	...
	%
	\begin{center}
			\includestandalone[width=.95\textwidth]{Pictures/hyb-image_BigDiagram_symplectic}
	\end{center}
	%
	\vfill
	\begin{minipage}[t][8em][t]{\textwidth}
	...
	\begin{tikzpicture}
		...
		\node [text width=30em,text centered] (L2) {
			...
			\begin{itemize}
				...
				\item<3->[•] It has a geometric interpration when $\omega$ is \alert{prequantizable}.
...
\begin{frame}[fragile]{Multisymplectic manifolds - mechanics perspective} %Fragile -->workaround tikzcd
	...
	\begin{table}
		\begin{tabular}{|p{0.2\textwidth}|p{0.3\textwidth}|p{0.35\textwidth}|} 
            ...
            &\onslide<2->{symplectic manifold}
            &\onslide<3->{multisymplectic manifold}
            \\[.25em]
            \parbox[][20pt][b]{0.2\textwidth}{classical \\ observables} 
            &\onslide<2->{Poisson algebra}
            &\onslide<3->{$L_\infty$-algebra}
            \\[.25em]
            \parbox[][20pt][b]{0.2\textwidth}{symmetries} 
            &\onslide<2->{group actions admitting comoment map}
            &\onslide<3->{group actions admitting (homotopy) comomentum map}
            \\
            \hline
  			\multicolumn{1}{c}{}
...
\begin{frame}{Hamiltonian forms}
	...
	\begin{columns}[T]
		...
		\begin{column}{.50\linewidth}
			...
			\vspace{1em}
			\begin{itemize}
				\item[\cmark] Skew-symmetric brakets;
		...
		\begin{column}{.50\linewidth}
			...
			\onslide<6->{
			\begin{itemize}
				\item[\cmark] Skew-symmetric brackets;
...
%-------------------------------------------------------------------------------------------------------------------------------------------------
\begin{frame}[fragile]{Vinogradov $L_\infty$-algebra}\label{frame:VinoLinfty}
	Vin. alg.oids 
	$\quad\simeq\quad$ 	
	...
	\only<3->{
		\tikz[overlay,remember picture]
		{
			\node[rounded corners,
            	 ...
            	 (base) at ($(current page.south)+(0,.5)$) [rotate=-0,text width=10cm,align=center] {
	...
	\hfill \onslide<6->{\hyperlink{frame:VinoDetails}{\color{gray}$[>>]$}}%\beamerskipbutton{}}
...
\begin{frame}[t]{Comoment maps}
	Consider a Lie algebra action $\underline{\,\cdot\,}:\mathfrak{g} \to \mathfrak{X}(M)$  preserving the $n$-plectic form $\omega$,
	\vfill
		...
		\begin{column}{.50\linewidth}
			...
			\onslide<2->{
				...
				\begin{defblock}[Comoment map pertaining to $v$]
					...
					such that
					$$ d~f (\xi) = -\iota_{\underline{\xi}} \omega \qquad \forall \xi \in \mathfrak{g}~.$$
				\end{defblock}
			}
		\end{column}	
		%
		...
		\begin{column}{.50\linewidth}
			...
			\onslide<3->{			
				...
				\begin{defblock}[Homotopy comoment map \tiny (HCMM)]
					...
					such that
					$$ d~f_1(\xi) = -\iota_{\underline{\xi}} \omega \qquad \forall \xi \in \mathfrak{g}~.$$
				\end{defblock}	
			}
		\end{column}	
	\end{columns}	
	...
	\begin{columns}[T]
		...
		\begin{column}{.50\linewidth}
			...
			\begin{propblock}[Noether Theorem]
				...
				$$\mathcal{L}_{\vHam_H} f(\xi) = 0 \qquad \forall \xi \in \mathfrak{g}$$
			\end{propblock}
			}
		\end{column}	
		%
		...
		\begin{column}{.50\linewidth}
			...
			\begin{propblock}[RWZ16 Theorem]
				...
				$$\mathcal{L}_{\vHam_H} f_k(p) \in B^k(M) \qquad \forall p \in Z_k(\mathfrak{g})$$			
			\end{propblock}
			}
		\end{column}	
	\end{columns}		
...
\begin{frame}{Gauge related multisymplectic manifolds and comoment maps}
	...
	\vfill
	%
	Consider \quad $\underline{\,\cdot\,} : \mathfrak{g} \to \mathfrak{X}(M)$ preserving $\omega$ and $\tilde{\omega}$,
	%\phantom{consider}\quad $\mathcal{L}_{v_\xi}\omega=\mathcal{L}_{v_\xi}\tilde{\omega} = 0 \quad \forall \xi \in \mathfrak{g}$
	\\
	let \qquad$f: \mathfrak{g}\to L_\infty(M,\omega)$ an \emph{homotopy comoment map} ~ w.r.t. $\mathfrak{g}\curvearrowright (M,\omega)$
	\pause
...
The pentagon commutes. 
	...
	\hfill \onslide<4->{\hyperlink{frame:thankyouslide}{\color{gray}$[\beamerskipbutton{}]$} }
...
\begin{frame}[fragile]{A bigger picture about Multisymplectic observables}
	%
	\begin{center}
		\includestandalone[width=.95\textwidth]{Pictures/Frame_OutlookDiagram}
	\end{center}		
...
%-------------------------------------------------------------------------------------------------------------------------------------------------
\begin{frame}[fragile]{Pentagonal Diagram and higher Prequantization}
	%
	\begin{center}
		\includestandalone[width=.8\textwidth]{Pictures/Frame_OutlookPrequantum}
	\end{center}		
	\onslide<3->{
		\small
		\begin{block}{Partial Results in the $2$-plectic case}
			\begin{itemize}
				\item G. Sevestre \& T. Wurzbacher, "On the Prequantisation Map for 2-Plectic Manifolds",  \cite{Sevestre2021};
				\item D. Djounvouna \& D. Krepski, "Infinitesimal symmetries of bundle gerbes and Courant algebroids"\cite{Djounvouna2023}.
			\end{itemize}
		\end{block}
	}
\end{frame}
...
\begin{frame}[fragile]{Why not a direct vertical arrow?}\label{frame:whyNoVertArrow}
	...
	\end{itemize}
	\hfill \onslide<4->{\hyperlink{frame:pentagonMotivation}{\color{gray}$[<<]$}}%\beamerskipbutton{}}
\end{frame}
%-------------------------------------------------------------------------------------------------------------------------------------------------
...
\begin{frame}[fragile]{Pentagonal diagram in the \emph{prequantizable} case \hfill (cont.)}
	...
	\hfill \onslide<5->{\hyperlink{frame:scopeTalk}{\color{gray}$[<<]$}}%\beamerskipbutton{}}
\end{frame}
%-------------------------------------------------------------------------------------------------------------------------------------------------
...
\begin{frame}[fragile]{A multisymplectic manifold in Lagrangian Field Theory}\label{frame:GIMMSY_construction}
	...
	%
	\vfill
	\hfill \onslide<4->{\hyperlink{frame:hystMS}{\color{gray}$[<<]$}}%\beamerskipbutton{}}
\end{frame}
%-------------------------------------------------------------------------------------------------------------------------------------------------
...
\begin{frame}[t,fragile,shrink=20]{Details on the Vinogradov $L_\infty$-algebra Construction}\label{frame:VinoDetails}
	...
	\end{enumerate}
			\hfill \onslide<5->{\hyperlink{frame:VinoLinfty}{\color{gray}$[<<]$}}%\beamerskipbutton{}}	
\end{frame}
%-------------------------------------------------------------------------------------------------------------------------------------------------
...
%##################################################################################
%-------------------------------------------------------------------------------------------------------------------------------------------------
% Include From M Zambon Talk.
\includepdf[pages={7-15}]{Sources/Zambon-slides.pdf}
%-------------------------------------------------------------------------------------------------------------------------------------------------
