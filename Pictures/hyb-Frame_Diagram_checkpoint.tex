...
\begin{document}
	\begin{tikzcd}[
		...
		|[visible on=<1>]| \color{grey}{(M,\omega)} 
		\ar[drr,grey,visible on=<1>,rightsquigarrow,"\text{observables}"]
		\ar[rrrr,grey,visible on=<1>,rightsquigarrow,"\text{twisted standard Vinogradov algebroid}"]
		\ar[dddd,grey,visible on=<1>,rightsquigarrow,sloped,"\text{gauge transf.}"']
		&[-1.5em] & &[2em] 
		&[-3em] |[visible on=<1>]| \color{grey}{(E^n,\rho,\langle\cdot,\cdot\rangle,[\cdot,\cdot]_\omega)}
		\ar[dl,grey,visible on=<1>,rightsquigarrow]
%		\ar[dddd,grey,visible on=<4->,sloped,"\cong","\text{B-transf.}"']
%
	\\
%
		& \phantom{\big(}
		& |[visible on=<1->,alias=A1]| L_{\infty}(M,\omega)
		\ar[r,visible on=<3->,line width=0.8pt,color=blue!60!black,"\Psi"]
		\ar[ddr,visible on=<5->,phantom,sloped,line width=0.8pt,color=red!60!black,"\mathbf\circlearrowright"]
		& |[visible on=<1->]| L_{\infty}\big(E^n,\omega\big)
		\ar[dd,sloped,line width=0.8pt,color=green!60!black,"\cong", visible on=<4->]
		&
%		 
	\\
%
		&|[alias=A2]|\mathfrak{g} \ar[ur,visible on=<1->,sloped,"(f)"]\ar[dr,visible on=<1->,sloped,"(\tilde{f})"']& & & 
%
	\\
%
...
%		
	\\
%	
		|[visible on=<1>]| \color{grey}{(M,\tilde{\omega})}
		\ar[rrrr,grey,visible on=<1>,rightsquigarrow,"\text{ }"]
		\ar[urr,grey,visible on=<1>,rightsquigarrow,"\text{ }"]
		& \phantom{\big(} & & 
